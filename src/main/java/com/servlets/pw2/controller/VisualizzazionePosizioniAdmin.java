...
@WebServlet("/VisualizzazionePosizioniAdmin")
public class VisualizzazionePosizioniAdmin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (risultatiRicerca != null && !risultatiRicerca.isEmpty()) {
			request.setAttribute("VisPosAdmin", risultatiRicerca);
		} else {
			ErrorManager.setOtherMessage("Non ho trovato posizioni sulla base dei criteri inseriti", request);
			request.setAttribute("VisPosAdmin", new ArrayList<Posizione>());
		}

		request.getRequestDispatcher("/admin/gestioneposizioni.jsp").forward(request, response);
	}
}
